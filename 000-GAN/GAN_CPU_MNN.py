...
for epoch in range(num_epoch):  # 进行多个epoch的训练
    for i, (img, _) in enumerate(dataloader):  # 469批图，一批图128张
        num_img = img.size(0)  # img,size():[128, 1, 28, 28]——128
        # view()函数作用是将一个多行的Tensor,拼接成一行
        # 第一个参数是要拼接的tensor,第二个参数是-1
        # =============================训练判别器==================
        img = img.view(num_img, -1)  # 将图片展开为28*28=784 #img.shape:[128, 784]
        real_img = Variable(img)  # 将tensor变成Variable放入计算图中 Variable变量:支持很多操作，将tensor比喻成鸡蛋，Variable就时装鸡蛋的篮子
